...
def custom_score(game, player):
    ...
    move_diff = float(my_moves - op_moves)

    # print('move_diff:', move_diff, 'center_dis', center_dis)
    return move_diff + my_moves
...
class MonteCarloTreeSearchPlayer(IsolationPlayer):
    def get_move(self, game, time_left):
        """Search for the best move from the available legal moves and return a
        result before the time limit expires.
    ...
    def get_move(self, game, time_left):
        ...
        **********************************************************************
        ...
        try:
            ...
            while True:
                depth += 1
                best_move = self.mcts(game, depth)
    ...
    def do_selection(self, game, depth):
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        legal_moves = game.get_legal_moves()
        if not legal_moves or depth == 0:
    ...
    def do_expansion(self, game, depth):
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        legal_moves = game.get_legal_moves()
        if not legal_moves or depth == 0:
    ...
    def do_simulation(self, game, depth):
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        legal_moves = game.get_legal_moves()
        if not legal_moves or depth == 0:
    ...
    def do_backpropagation(self, game, depth):
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        legal_moves = game.get_legal_moves()
        if not legal_moves or depth == 0:
